...
@Component({
    selector: 'app-poule-match',
    imports: [
        MatchView,
        ConfirmDialog,
        Button
    ],
    templateUrl: './poule-match.html',
    styleUrl: './poule-match.css',
})
export class PouleMatch extends Base implements OnInit {
    ...
    override ngOnInit(): void {
        ...
        .then(results => {
            ...
            .then(data => {
                ...
                if (splKopIdx < 0) {
                    this.alert.showError(`PouleMatch : Koppel met ID '${splKopId}' niet gevonden in poule koppels.`);
                    return;
                }
                const tegKopIdx = this.poule.koppels.findIndex(kpl => kpl.pouleKplId == tegKopId);
                if (tegKopIdx < 0) {
                    this.alert.showError(`PouleMatch : Koppel met ID '${tegKopId}' niet gevonden in poule koppels.`);
                    return;
                }
                this.koppels.push(this.poule.koppels[splKopIdx]);
                this.koppels.push(this.poule.koppels[tegKopIdx]);
                this.header.subtitle = `Seizoen ${this.header.seizoen} - ${this.ronde.rndNaam} - Match poule ${this.poule.id} : ` + 
                                        `koppels ${this.koppels[0].pouleKplId} - ${this.koppels[1].pouleKplId}`;
